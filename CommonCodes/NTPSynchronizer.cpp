...
CNTPSynchronizer::CNTPSynchronizer(void)
{
	...
	mNoOfRequest = 100;
	mServerAddress = ADDR_SERVER;
}

CNTPSynchronizer::CNTPSynchronizer(std::string strName, int nRequest)
{
	pNTPTask = this;
	mStopSignal = false;
	mNTPTimestamp = 0;
	mNtpTImeOffset = 0;
	mIsReadyToComputeOffset = false;
	mServerAddress = strName;
	mNoOfRequest = nRequest;
...
bool CNTPSynchronizer::runNTPSynchronization()
{
	...
	device[0] = "NTPClient";
	if(mNtpTask.Init(mServerAddress, device, "NTPClient", "", onPacketReceivedForClient)){
		...
		if(head == '#'){
			...
			if(nBufLen == 1+8){
				...
				if(roundtrip_delay < 800000){ // 80ms
					...
					if(mCountRecivedData >= mNoOfRequest){
