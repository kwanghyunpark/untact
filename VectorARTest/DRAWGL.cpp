...
void DRAWGL::Canvas1(double* pos1, double* pos2, double* gBallPosition, double gBallRadius)
{
	...
	//----Camera Pos----
	camera1.setPosition(ofVec3f(0,0.2,0.4)*scale);  // distance between camera and target object is 80cm
	camera1.lookAt(ofVec3f(0.0f,0.1f,0.0f)*scale, ofVec3f(0.0f,1.0f,0.0f));
	//ofTranslate(0,0,0);
	ofDisableLighting();
	ofEnableLighting();
	...
	gluSphere( gluQuadps1, 0.01*scale, 10, 10 ); // radius of sphere size is 2cm 
	ofPopMatrix();
	...
	ofTranslate(pos2[0]*scale, pos2[1]*scale, pos2[2]*scale);
	
	ofSetHexColor(0x0000FF);
	gluSphere( gluQuadps2, 0.01*scale, 10, 10 );
	ofPopMatrix();
	...
	ofTranslate(gBallPosition[0]*scale, gBallPosition[1]*scale, gBallPosition[2]*scale);

	ofSetHexColor(0x00FFFF);
	//glutSolidCube(2*gBallRadius*scale);
	ofPopMatrix();
	...
	glScalef(4,1,1);
	glutWireCube(2*gBallRadius*scale);
	ofPopMatrix();
